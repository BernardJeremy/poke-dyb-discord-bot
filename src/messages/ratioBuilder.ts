...
const buildRatio = () => {
  ...
  allUsersStats.sort((a, b) => (
    ...
    exampleEmbed.addFields(
      { name: userStat.name, value: (`[${userStat.nbrPokemon}/${completePokedex.length}]  ${userStat.invocSuccessPercent}% Invoc new / ${userStat.towerSuccessPercent}% Tour / ${userStat.safariSuccessPercent}% Safari`) },
    );
  });
  ...
  exampleEmbed.addFields(
    { name: 'EN MOYENNE', value: (`${averageRatio.invocSuccessPercentage}% Invoc new / ${averageRatio.towerSuccessPercentage}% Tour / ${averageRatio.safariSuccessPercentage}% Safari`) },
  );
